...
static const struct TypeStrOfId {
  ...
  { Frame::FT_EncoderSettings, I18N_NOOP("TSSE - Software/Hardware and settings used for encoding"), true },
  ...
  if (m_tagV2) {
    ...
    if ((id3v2Tag = dynamic_cast<TagLib::ID3v2::Tag*>(m_tagV2)) != 0) {
      ...
      for (TagLib::APE::ItemListMap::ConstIterator it = itemListMap.begin();
        ...
        frames.insert(
          Frame(type, values.size() > 0 ? TStringToQString(values.front()) : QLatin1String(""),
                name, i++));
      }
#if TAGLIB_VERSION >= 0x010600
#ifdef TAGLIB_WITH_MP4
